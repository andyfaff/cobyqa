...
def spider_geometry(const, grad, curv, xpt, xl, xu, delta, debug):
    ...
    for k in range(xpt.shape[1]):
        ...
        alpha_xl_pos = np.max(xl[i_xl_pos[k]] / xpt[i_xl_pos[k], k], initial=-np.inf)
        alpha_xl_neg = np.min(xl[i_xl_neg[k]] / xpt[i_xl_neg[k], k], initial=np.inf)
        alpha_xu_neg = np.max(xu[i_xu_neg[k]] / xpt[i_xu_neg[k], k], initial=-np.inf)
        alpha_xu_pos = np.min(xu[i_xu_pos[k]] / xpt[i_xu_pos[k], k], initial=np.inf)
